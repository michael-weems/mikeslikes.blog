...
export async function renderInCommandPalette(){
  whenElementReady('#search-bar').then( (searchBar) => {
    ...
    searchBar.addEventListener('keyup', async () => {
      ...
      fetch(`${process.env.API_ADDRESS}/search?query=${text}`).then(response => response.json()).then(async (response: {posts: SearchResult[]}) => {
        ...
        articleLinks.forEach((element) => element.addEventListener('click', () => {
          ...
          closeCommandPalette().then(() => {
            ...
            fetch(`${process.env.API_ADDRESS}/article-html?article=${articleTitle}`).then(response => response.json()).then((response: {article: string}) => {
